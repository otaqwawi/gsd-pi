...
import { expandPromptTemplate, type PromptTemplate } from "@gsd/pi-coding-agent/core/prompt-templates.js";
import type { ResourceExtensionPaths, ResourceLoader } from "@gsd/pi-coding-agent/core/resource-loader.js";
import type { BranchSummaryEntry, CompactionEntry, SessionManager } from "@gsd/pi-coding-agent/core/session-manager.js";
...
import { createSyntheticSourceInfo, type SourceInfo } from "@gsd/pi-coding-agent/core/source-info.js";
...
export class AgentSession {
	...
	private _visibleSkillNames?: string[];
		...
		runner.bindCore(
			{
				...
				getVisibleSkills: () => this._visibleSkillNames,
				setVisibleSkills: (skillNames) => {
					this._visibleSkillNames = skillNames;
				},
				emitBeforeModelSelect: (event) => this._extensionRunner.emitBeforeModelSelect(event),
				emitAdjustToolSet: (event) => this._extensionRunner.emitAdjustToolSet(event),
				emitExtensionEvent: (event) => this._extensionRunner.emitExtensionEventDynamic(event),
			},
			{
				getModel: () => this.model,
	...
	/** Case-insensitive lookup for custom tool render definitions. */
	getRenderableToolDefinition(toolName: string): ToolDefinition | undefined {
		const normalized = toolName.toLowerCase();
		for (const { definition } of this._extensionRunner.getAllRegisteredTools()) {
