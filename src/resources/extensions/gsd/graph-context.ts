...
async function resolveGraphApi(): Promise<GraphApi> {
  ...
  try {
    const imported = await import("@opengsd/mcp-server");
    if (isGraphApi(imported)) {
      cachedGraphApi = imported;
      return cachedGraphApi;
    }
    logWarning("prompt", "@opengsd/mcp-server graph exports unavailable; using local graph fallback");
  } catch {
 ...
 *
 * Returns null when:
 * - @opengsd/mcp-server fails to import
